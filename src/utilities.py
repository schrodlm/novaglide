...
def draw_text(text: str, size: int, x: int, y: int, display: pygame.Surface,
    ...
    ------
    TypeError
        When incorrect input parameters are passed
    OutOfBoundsError
        When the color is passed as a tuple but exceeding the possible range.
    InvalidColorString
        If the color is passed as text but not known by pygame.
    """
    if not isinstance(text, str) or not isinstance(display, pygame.Surface):
        raise TypeError("Incorrect type of the parameters.")
    for integer in (x, y, size):
        if not isinstance(integer, int):
            raise TypeError("Incorrect type of the parameters.")

    if isinstance(color, str):
        check_string_color_posibility(color)
    else:
        check_color_values(color[0], color[1], color[2])
    font = get_font(size)
...
def check_color_values(r: int, g: int, b: int):
    ...
    if any(color < 0 for color in [r, g, b]) or any(color > 255 for color in [r, g, b]):
